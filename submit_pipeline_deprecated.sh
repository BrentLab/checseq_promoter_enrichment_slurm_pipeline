...
#
# Usage: submit_pipeline.sh <lookup_file> [--bam-type=nuclear|full] [--start-at=STEP]
#
...
if [[ -z "${LOOKUP_FILE}" ]]; then
    echo "ERROR: lookup_file is required. Usage: submit_pipeline.sh <lookup_file> [--bam-type=nuclear|full] [--start-at=STEP]"
    exit 1
fi
