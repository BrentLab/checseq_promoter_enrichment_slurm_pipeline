...
# CONFIGURATION
# ============================================================================
# Usage: 02_maketagdir_samples.sh <lookup_file> [bam_type]
...
#   Fragment length is derived per-sample at runtime from that sample's own
#   samtools_stats.txt "insert size average" (produced by 01_align.sh),
#   rather than a single shared value -- each regulator's ChEC-seq fragment
#   size can genuinely differ from the free MNase control.
LOOKUP_FILE="$1"
BAM_TYPE="${2:-nuclear}"
...
#                      HOMER's autocorrelation estimate
makeTagDirectory "${TAG_DIR}" "${TEMP_SAM}" \
    -format sam -unique -mapq 10 -genome "${GENOME_FASTA}" -checkGC \
