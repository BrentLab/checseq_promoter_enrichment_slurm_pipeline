...
option_list <- list(
    ...
    make_option("--output-dir", type = "character", default = "results/promoter_scoring/mahendrawada_hahn",
...
read_summits <- function(regulator, replicate, peaks_dir) {
    ...
    if (!file.exists(path) || file.size(path) == 0) {
        message(sprintf("  No summits file (or empty) for %s_%s", regulator, replicate))
        return(GRanges())
    }
